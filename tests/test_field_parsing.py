...
def test_match_brand_name_finds_brand_tokens_scattered_across_lines():
    # Real-label pattern: 'ROCK' and 'TOWN' separated by header junk.
    lines = [
        OcrLine(text="BATCH", confidence=0.99, bbox=[], y_center=0, x_center=0),
        OcrLine(text="ROCK", confidence=0.99, bbox=[], y_center=1, x_center=0),
        OcrLine(text="EST.D", confidence=0.99, bbox=[], y_center=2, x_center=0),
        OcrLine(text="2010", confidence=1.00, bbox=[], y_center=3, x_center=0),
        OcrLine(text="TOWN", confidence=1.00, bbox=[], y_center=4, x_center=0),
        OcrLine(text="ARKANSAS", confidence=0.99, bbox=[], y_center=5, x_center=0),
    ]
    result = match_brand_name(lines, "Rock Town")
    assert result.status == "match"


def test_match_brand_name_typo_in_near_length_line():
    # OCR drops a char: 'Darros' vs expected 'Darroze'. Length filter must
    # allow a small shortfall so near-length typos still land.
    lines = [
        OcrLine(text="BAS-ARMAGNAC", confidence=0.99, bbox=[], y_center=0, x_center=0),
        OcrLine(text="Darros", confidence=0.87, bbox=[], y_center=1, x_center=0),
    ]
    result = match_brand_name(lines, "Darroze")
    assert result.status == "match"


def test_match_brand_name_rejects_wrong_multi_token_brand():
    # Expected 'OLD FOX DISTILLERY' vs OCR 'OLD TOM DISTILLERY'. partial_ratio
    # scores ~89 because prefix+suffix match, but the middle token is wrong.
    # The per-token guard must reject this — otherwise wrong-SKU labels would
    # silently pass as match.
    lines = [
        OcrLine(text="OLD TOM DISTILLERY", confidence=0.95, bbox=[], y_center=0, x_center=0),
    ]
    result = match_brand_name(lines, "OLD FOX DISTILLERY")
    assert result.status == "mismatch"
